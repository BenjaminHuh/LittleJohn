...
class StockChart extends React.Component { 
    ...
    priceUpdate({ payload, label, active }) {
        ...
        if (active && payload[0]) {
            ...
            // debugger;
            document.getElementById("currChange").innerHTML = `${dSign}${Math.abs(regularMarketPreviousClose - payload[0].value).toFixed(2)} (${dSign}${Math.abs((regularMarketPreviousClose - payload[0].value)/regularMarketPreviousClose * 100).toFixed(2)}%) at ${payload[0].payload.label} EST`
        }
